...
\033[1;37m FACEBOOK:\033[1;32m  Nguyễn Văn Dững(donny)
\033[1;37m TELEGRAM:\033[1;32m  t.me/xyzvandung
\033[1;37m GMAIL:\033[1;32m     donnyleaderx22@gmail.com
\033[1;32m===================================================
"""
for h in banner:
       sys.stdout.write(h)
...
if check_file_key == False:
   ...
   while(True):
      ...
      if ma == key:
         print(f'{mquang}{luc}KEY CHÍNH XÁC')
         luu=open(file_key, 'a+')
         luu.write(ma)
         luu.close()
         break
      elif ma != key:
         print(f'{mquang}{do}KEY SAI')
elif check_file_key == True:
    print(f'{mquang}{luc}VUI LÒNG ĐỢI LOGIN KEY',end='\r')
    sleep(1)
    k=open(file_key, 'r')
    ma=k.read()
    k.close()
    if ma == key:
        print(f'{mquang}{luc}KEY CHÍNH SÁC       ',end='\r')
        sleep(0.5)
    elif ma != key:
        if os.path.exists(file_key_cu) == True:
            os.system(f'rm {file_key_cu}')
        os.system(f'rm {file_key}')
        print(f'{mquang}{do}API Key Sai         ')
        while(True):
            ma=input(f"{mquang}{luc}Nhập API Key\033[1;32m Ngày \033[1;37m{today}: \033[1;33m")
            if ma == key:
                print(f'{mquang}{luc}KEY CHÍNH XÁC')
                luu=open(file_key, 'a+')
                luu.write(ma)
...
amount = 5000 
print(f'\033[1;36m║\033[1;33mNgày Và Giờ  \033[1;37m{thoigian}\033[1;36m')
...
def run(sdt,i):                  
  ...
  threading.submit(KILO,sdt)
  threading.submit(GAPO,sdt) 
  print("[ DONNY [SPAM CALL,SMS DELAY 1s THÀNH CÔNG]",)  
  for j in range(0, 5):
    # code trong vòng lặp
    print(f" DONNY | TOOL SPAM CALL SMS \r",end="")
